			...
			while (keepOn) {
				...
				case Message.LOGOUT:
					...
					broadcast(new Message(Message.USER, username
							+ "updateTheUser", null, chatMsg.getRecipient(), chatMsg.getSender()));
				...
				case Message.USER:
					...
					for (int i = 0; i < clientList.size(); ++i) {
						...
						broadcast(new Message(Message.USER, ct.username
								+ "USERNAME100", null, chatMsg.getRecipient(), chatMsg.getSender()));
					}
						...
						for (Message mezz : offMsg){
							for (String s : mezz.getRecipient()){
								if(s.equals(username)){
									logHandler("Offline user has logged in, pushing new message!");
									broadcast(new Message(Message.MESSAGE,  mezz.getMessage(), mezz.getImage(), mezz.getRecipient(), mezz.getSender()));
								}
							}
						}
					}
				...
				case Message.OFFLINE:
					offMsg.add(chatMsg);
					logHandler("User was offline, a message has been stored on server!");
